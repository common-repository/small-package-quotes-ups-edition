...
class Eniture_EnUpsSmallFdo
{
    ...
    public function en_cart_package($package)
    {
        ...
        $this->en_fdo_meta_data['plugin_name'] = 'ups_small';
        $accessorials['residential'] = get_option('ups_small_quote_as_residential_delivery') == 'yes' ? true : false;
    ...
    public function en_package_items($items)
    {
        ...
        foreach ($items as $item_key => $item_data) {
            $productId = $productName = $productQty = $actualProductPrice = $products = $productPrice = $productWeight = $productLength = $productWidth = $productHeight = $ptype = $hazardousMaterial = $productType = $productSku = $productClass = $attributes = $variantId = $markup = '';
            $attributes = [];
            extract($item_data);
