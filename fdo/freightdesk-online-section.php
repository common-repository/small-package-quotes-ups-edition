<?php
/**
 * UPS Freightdesk online Template
 *
 * @package     UPS Quotes
...
if(get_option('en_fdo_company_id_status') != 1) {
    $button_connect = '<a href="javascript:void(0)" id="fd_online_id_ups_s" class="button-primary">Connect</a>';
    ...
    if(get_option('en_fdo_company_id_status') == 1) {
        $button_connect = '<a href="javascript:void(0)" data="disconnect" id="fd_online_id_ups_s" class="button-primary">Disconnect</a>';
...
$eniture_ups_small_freightdesk_fields = wp_sprintf('<div style="%s">
                            %s
                         </div> 
                         <span id="con_dis"> %s</span>
                         %s
                        </div></div>', $style, $id_field, $button_connect, $ulli);
echo $eniture_ups_small_freightdesk_fields;
?>