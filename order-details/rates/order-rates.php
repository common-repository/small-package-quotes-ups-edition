 ...
 */
if (!class_exists('Eniture_UpsSpqOrderRates')) {

    class Eniture_UpsSpqOrderRates
    {
        ...
        public function en_order_before_calculate_totals($and_taxes, $order)
        {
            ...
            if (isset($woocommerce->cart)) {
                    ...
                    foreach ($shipping as $item_id => $per_shipment) {
                        ...
                        switch ($method_id) {
                            case 'ups_small':
                                ...
                                if (empty($errors)) {
                                    $shipping_class = new Eniture_WC_ups_small();
                                    ...
                                    if (isset($response['cost']) && $response['cost'] > 0) {
                                        ...
                                        $per_shipment->update_meta_data('en_free_rate_details', wp_json_encode([]));
                ...
                if ($order_recreated) {
                    ...
                    $ups_curl_obj = new Eniture_UPS_Small_Curl_Request();
                    $ups_curl_obj->ups_small_get_curl_response(ENITURE_UPS_SPQ_FDO_HITTING_URL, $request_data);
                }
            }
        }
    }

    new Eniture_UpsSpqOrderRates();
}