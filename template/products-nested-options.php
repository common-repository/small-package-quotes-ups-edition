...
if (!class_exists("Eniture_UpdateProductNestedOptions")) {

    class Eniture_UpdateProductNestedOptions
    {
        ...
        function en_product_custom_nested_fields($post_id)
        {
            ...
            if (isset($plan_status['nesting'])) {
                if (!in_array(0, $plan_status['nesting']['plan_required'])) {
                    ...
                    $description = apply_filters("eniture_ups_small_plans_notification_link", [3]);
...
//          Nested Material checkbox
            $field_array = array(
                ...
                'label' => __(
                    'Nested material', 'small-package-quotes-ups-edition'
                ),
                'value' => get_post_meta(
...
//          Nesting percentage input
            $field_array = array(
                ...
                'label' => __(
                    'Nesting(%)', 'small-package-quotes-ups-edition'
                ),
                'desc_tip' => true,
                'description' => __("How much of the item can be nested into another. Default 0, Range from 0 to 100.", "small-package-quotes-ups-edition"),
                'value' => get_post_meta(
...
//          Nested Dimension dropdown
            $field_array = array(
                ...
                'label' => __(
                    'Nested dimension', 'small-package-quotes-ups-edition'
                ),
                'desc_tip' => true,
                'value' => $nestedDimensionSelected,
                'description' => __("This setting identifies which dimension will be used for the nesting property.", "small-package-quotes-ups-edition"),
                'options' => array(
                    'length' => 'Length',
                    'width' => 'Width',
                    'height' => 'Height',
...
//          Maximum nested items input field
            $field_array = array(
                ...
                'label' => __(
                    'Maximum nested items', 'small-package-quotes-ups-edition'
                ),
                'desc_tip' => true,
                'description' => __('It represents the maximum number of items that can be nested into one another.', 'small-package-quotes-ups-edition'),
                'value' => get_post_meta(
...
//          Stacking property dropdown
            $field_array = array(
                ...
                'label' => __(
                    'Stacking property', 'small-package-quotes-ups-edition'
                ),
                'desc_tip' => true,
                'value' => $nestedStakingPropertySelected,
                'description' => __('This setting identifies how the nested stacks should be handled if the total number of items exceeds the maximum.', 'small-package-quotes-ups-edition'),
                'options' => array(
                    'Evenly' => 'Evenly',
                    'Maximized' => 'Maximized',
                ),
    ...
    /* Initialize object */
    new Eniture_UpdateProductNestedOptions('hooks');
}