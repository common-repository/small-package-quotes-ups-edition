 ...
 */
if (!function_exists("eniture_woo_wd_admin_script_style")) {
    function eniture_woo_wd_admin_script_style()
    {
        ...
        wp_register_style('warehouse_section', plugin_dir_url(__FILE__) . '/wild/assets/css/warehouse_section.css', false, '1.0.4');
        wp_enqueue_style('warehouse_section');
    }

    add_action('admin_enqueue_scripts', 'eniture_woo_wd_admin_script_style');
}

/**
 * Warehouse Template
 */
if (!function_exists('eniture_warehouse_template')) {
    function eniture_warehouse_template($action = FALSE)
    {
        ...
        if (is_array($multi_warehouse) && count($warehous_list) > 0) {
            ...
            $multi_warehouse_package_required = apply_filters("eniture_ups_small_plans_notification_link", $multi_warehouse);
        }
        ?>
        ...
        <a href="#en_wd_add_warehouse_btn" onclick="en_wd_add_warehouse_btn()" title="Add Warehouse"
           class="en_wd_add_warehouse_btn <?php echo esc_attr($multi_warehouse_disabled);
           ?>" name="avc">Add</a>
        ...
        <table class="en_wd_warehouse_list" id="append_warehouse">
            ...
            if (count($warehous_list) > 0) {
                ...
                foreach ($warehous_list as $list) {
                    ?>
                    <tr class="<?php echo (strlen($tr_disabled_me) > 0 && $count != 0) ? esc_attr($tr_disabled_me) : ""; ?>"
                        ...
                        <td class="en_wd_warehouse_list_data">
                            <a href="javascript(0)"
                               onclick="return eniture_wd_edit_warehouse(<?php echo (isset($list->id)) ? esc_attr($list->id) : ''; ?>);"><img
                                        src="<?php echo esc_url_raw(plugins_url()); ?>/small-package-quotes-ups-edition/warehouse-dropship/wild/assets/images/edit.png"
                                        title="Edit"></a>
                            <a href="javascript(0)"
                               onclick="return en_wd_delete_current_warehouse(<?php echo (isset($list->id)) ? esc_attr($list->id) : ''; ?>);"><img
                                        src="<?php echo esc_url_raw(plugins_url()); ?>/small-package-quotes-ups-edition/warehouse-dropship/wild/assets/images/delete.png"
                                        title="Delete"></a>
                        </td>
                    </tr>
                    <?php
 ...
 * Dropship Template
 */
if (!function_exists('eniture_dropship_template')) {
    function eniture_dropship_template($action = FALSE)
    {
        ...
        if (is_array($multi_dropship) && count($dropship_list) > 0) {
            ...
            $tr_disabled_me = "tr_disabled_me";
            $multi_dropship_package_required = apply_filters("eniture_ups_small_plans_notification_link", $multi_dropship);
        }
        ?>

        <div class="add_btn_dropship">
        <a href="#add_dropship_btn" onclick="hide_drop_val()" title="Add Drop Ship"
           class="en_wd_add_dropship_btn hide_drop_val <?php echo esc_attr($multi_dropship_disabled); ?>">Add</a>
            ...
            if (count($dropship_list) > 0) {
                ...
                foreach ($dropship_list as $list) {
                    ...
                    <tr class="<?php echo (strlen($tr_disabled_me) > 0 && $count != 0) ? esc_attr($tr_disabled_me) : ""; ?>"
                        ...
                        <td class="en_wd_dropship_list_data">
                            <a href="javascript(0)"
                               onclick="return eniture_wd_edit_dropship(<?php echo (isset($list->id)) ? esc_attr($list->id) : ''; ?>);"><img
                                        src="<?php echo esc_url_raw(plugins_url()); ?>/small-package-quotes-ups-edition/warehouse-dropship/wild/assets/images/edit.png"
                                        title="Edit"></a>
                            <a href="javascript(0)"
